...
<div class="jumbotron rounded-0 mb-0 border-bottom">
    <h1 class="text-center py-5">Ready To Deliver</h1>
    <div class="container">
        <table class="table table-bordered py-5 text-center">
            ...
            <tbody>
                ...
                @if($commande->cstate->name == 'ready' and $commande->ctype->name == 'delivery' )
                <tr>
                <td>{{$commande->id}}</td>
                ...
                <td>
                    {!! Form::model($commande,['method'=>'PATCH','action'=>['DeliveryController@update',$commande->id]])  !!}
                    <input type="hidden" name="cstate_id" value="5">
                        
                        {!! Form::submit('Deliver',['class'=>'btn btn-outline-primary']) !!}
                        {!! Form::close() !!}
                        </td>
                </tr>
                @endif
...
<div class="jumbotron rounded-0 mb-0 pt-0 border-bottom">
    ...
    <div class="container">
        <table class="table table-bordered py-5 text-center">
            ...
            <tbody>
                @foreach($data as $commande)
                @if($commande->cstate->name == 'delivering' and $commande->ctype->name == 'delivery' )
                <tr>
                <td>{{$commande->id}}</td>
                <td>0{{$commande->number}}</td>
                <td>@foreach($commande->order as $ord)
                ...
                <td>
                    {!! Form::model($commande,['method'=>'PATCH','action'=>['DeliveryController@update',$commande->id]])  !!}
                    <input type="hidden" name="cstate_id" value="2">
                        
                        {!! Form::submit('Delivered',['class'=>'btn btn-outline-success']) !!}
                        {!! Form::close() !!}
                </td>
                </tr>
                @endif
    ...
    <div class="container">
        <table class="table table-bordered py-5 text-center">
            ...
            <tbody>
                ...
                @if($commande->cstate->name == 'delivered' and $commande->ctype->name == 'delivery' )
                <tr>
                <td>{{$commande->id}}</td>
                <td>0{{$commande->number}}</td>
                <td>@foreach($commande->order as $ord)
